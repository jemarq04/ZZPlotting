...
def getStorageArea():
    ...
    if "storage_area" not in config['Setup'] or "html_area" not in config['Setup'] :
        raise ValueError("storage path not specified in config file %s"
                        % config_name)
...
def getListOfFiles(filelist, selection, manager_path="", analysis=""):
    ...
    for name in filelist:
        if ".root" in name:
            names.append(name)
        elif any("ZZ4l%s" % year in name for year in ["2022","2023","Run3Combined"]):
            key = ["ZZ4l%s" % year for year in ["2022","2023","Run3Combined"] if "ZZ4l%s" % year in name][0]

...
def makePlots(hist_stacks, data_hists, name, args, signal_stacks=None, errors=None):
    ...
    ROOT.gStyle.SetLineWidth(
        3
    )  # For hists created before this command, line width not affected, if created after then affected
    first = True
    for hist_stack, data_hist, signal_stack in zip(hist_stacks, data_hists, signal_stacks):
        print("makePlot called")
        makePlot(hist_stack, data_hist, name, args, signal_stack, same=(" same" if not first else ""))
        first = False
    offset = ROOT.gPad.GetLeftMargin() - 0.04 if args.legend_left else ROOT.gPad.GetRightMargin() - 0.04
    if hasattr(args, "selection"):
        width = 0.2 if "ZZ4l" in args.selection else 0.33
    else:
        width = 0.33
    width *= args.scalelegx
    xdist = 0.1 if args.legend_left else 0.91
    xcoords = [xdist + offset, xdist + width + offset] if args.legend_left else [xdist - width - offset, xdist - offset]
    ...
    if not args.no_decorations:
        ...
        scale_label = (
            "Normalized to Unity" if args.luminosity < 0 else "%s fb^{-1}" % int(round(float(args.luminosity)))
        )
    ...
    if args.extra_text != "" or glb_var in [
        "jetPt[0]",
        "jetPt[1]",
        "jetEta[0]",
        "jetEta[1]",
    ...
    if args.scatter:
        ...
        canvas = plotter.splitCanvasWithSyst(
            ratioband,
            canvas,
            canvas_dimensions,
            "#scale[0.85]{Data / Pred.}" if data_hists[0] else args.ratio_text,
...
def makePlot(hist_stack, data_hist, name, args, signal_stack=0, same=""):
    ...
    stack_drawexpr = " ".join(["hist"] + ["nostack" if args.nostack else ""])
    ...
    if signal_stack:
        ...
        signal_stack.GetHistogram().GetXaxis().SetTitle(hists[0].GetXaxis().GetTitle())
    ...
    if not args.scatter:
        ...
        first_stack.GetYaxis().SetTitle(hists[0].GetYaxis().GetTitle() + ("/bin" if not glb_isFullMass else "/GeV"))
    else:
        hists[0].GetYaxis().SetTitle(hists[0].GetYaxis().GetTitle() + ("/bin" if not glb_isFullMass else "/GeV"))
    ...
    if not args.scatter:
        ...
        first_stack.GetHistogram().GetXaxis().SetTitle(hists[0].GetXaxis().GetTitle())
...
def getHistFactory(
    config_factory, selection, filelist, luminosity=1, hist_file=None, unweighted=False, lhe_weight_id=None
):
    ...
    for name in filelist:
        ...
        if name not in list(all_files.keys()):
            if hist_file is not None and not hist_file.Get(name):
                logging.warning(
                    "%s is not a valid file name (must match a definition in FileInfo/%s.json)" % (name, selection)
                )
                continue
        ...
        if "data" not in name.lower() and name != "nonprompt":
            # pdb.set_trace()
            kfac = 1.0 if "kfactor" not in list(mc_info[base_name].keys()) else mc_info[base_name]["kfactor"]
            if not hist_file:
                metaTree = ROOT.TChain(metaTree_name)
                metaTree.Add(hist_factory[name]["file_path"])
                weight_info = WeightInfo.WeightInfoProducer(
                    metaTree, mc_info[base_name]["cross_section"] * kfac if not unweighted else 1, sum_weights_branch
                ).produce()
            else:
...
def histWithScaleUnc(scale_hist2D, entries, name):
    ...
    for i in range(1, scale_hist.GetNbinsX() + 1):
        try:
            maxScale = max([h.GetBinContent(i) for h in hists if not h.GetBinContent(i) == 0])
            minScale = min([h.GetBinContent(i) for h in hists if not h.GetBinContent(i) == 0])
...
def getScaleFactorExpressionMedTightWElec(state):
    if state == "eee":
        return "e1MediumIDSF*e2MediumIDSF*e3TightIDSF*pileupSF"
    elif state == "eem":
        return "e1MediumIDSF*e2MediumIDSF*mTightIsoSF*mMediumIDSF*pileupSF"
    elif state == "emm":
        return "eTightIDSF*m1MediumIDSF*m1TightIsoSF*m2TightIsoSF*m2MediumIDSF*pileupSF"
    elif state == "mmm":
        return "m1TightIsoSF*m1MediumIDSF*m2TightIsoSF*m2MediumIDSF*m3TightIsoSF*m3MediumIDSF*pileupSF"


def getScaleFactorExpressionAllTight(state):
    if state == "eee":
        return "e1TightIDSF*e2TightIDSF*e3TightIDSF*pileupSF"
    elif state == "eem":
        return "e1TightIDSF*e2TightIDSF*mTightIsoSF*mTightIDSF*pileupSF"
    elif state == "emm":
        return "eTightIDSF*m1TightIDSF*m1TightIsoSF*m2TightIsoSF*m2TightIDSF*pileupSF"
    elif state == "mmm":
        return "m1TightIsoSF*m1TightIDSF*m2TightIsoSF*m2TightIDSF*m3TightIsoSF*m3TightIDSF*pileupSF"
...
def getPlotPaths(selection, folder_name, write_log_file=False):
    ...
    plot_path = "/".join(
        [base_dir, selection]
        + (
            ["{:%Y-%m-%d}".format(datetime.datetime.today()), "{:%Hh%M}".format(datetime.datetime.today())]
            if folder_name == ""
            else [folder_name]
        )
    )
...
def getGenChannelCut(channel):
    ...
    if channel == "eem":
        cut_string = (
            "((abs(l1pdgId) == 11 && abs(l2pdgId) == 11 && abs(l3pdgId) == 13)"
            " || (abs(l1pdgId) == 11 && abs(l2pdgId) == 13 && abs(l3pdgId) == 11)"
            " || (abs(l1pdgId) == 13 && abs(l2pdgId) == 11 && abs(l3pdgId) == 11))"
        )
    elif channel == "emm":
        cut_string = (
            "((abs(l1pdgId) == 13 && abs(l2pdgId) == 13 && abs(l3pdgId) == 11)"
...
def savePlot(canvas, plot_path, html_path, branch_name, write_log_file, args):
    ...
    if not args.no_html:
        ...
        subprocess.call(
            ["epstopdf", "--outfile=%s" % output_name + ".pdf", output_name + ".eps"], env={"PATH": os.environ["PATH"]}
        )
...
def getSystValue(hMain):
    ...
    alldata, dataSumW = HistTools.makeCompositeHists(
        fOut,
        "AllData",
        ConfigureJobs.getListOfFilesWithXSec([analysis + "data"], manager_path),
        func_lumi,
        underflow=False,
        overflow=False,
    )
    ...
    ewkmc, ewkSumW = HistTools.makeCompositeHists(
        fOut,
        "AllEWK",
        ConfigureJobs.getListOfFilesWithXSec(ConfigureJobs.getListOfEWKFilenames(analysis), manager_path),
        func_lumi,
        underflow=False,
        overflow=False,
    )
    ...
    ewkmc_qqZZonly, ewkSumW_qqZZonly = HistTools.makeCompositeHists(
        fOut,
        "AllEWKqqZZonly",
        ConfigureJobs.getListOfFilesWithXSec(ConfigureJobs.getListOfEWKFilenames(analysis)[:1], manager_path),
        func_lumi,
        underflow=False,
        overflow=False,
    )
    ...
    allVVVmc, VVVSumW = HistTools.makeCompositeHists(
        fOut,
        "AllVVV",
        ConfigureJobs.getListOfFilesWithXSec(ConfigureJobs.getListOfVVV(analysis), manager_path),
        func_lumi,
        underflow=False,
        overflow=False,
    )
    ...
    for chan in channels:
        ...
        # Trigger efficiency
        TrigUnc = 0.02
        TrigScale = {"Up": 1.0 + TrigUnc, "Down": 1.0 - TrigUnc}
        for syst, scale in TrigScale.items():
            ...
            hChangeTrig = hNoFakeTrig * (scale - 1.0)
            # hChangeLumi.SetDirectory(0)
    ...
    for i in range(1, tmpData.GetNbinsX() + 1):
        if hMain.GetBinContent(i) == 0:
            continue
        bw = tmpData.GetBinWidth(i) if normBW else 1.0
    ...
    for i in range(1, tmpData.GetNbinsX() + 1):
        if hMain.GetBinContent(i) == 0:
            ...
            ratioGraph.SetPointEYhigh(i - 1, 0.0)
            ratioGraph.SetPointEYlow(i - 1, 0.0)
            continue
        bw = tmpData.GetBinWidth(i) if normBW else 1.0
        eUp = hUncUp.GetBinContent(i) / bw
        eDn = hUncDn.GetBinContent(i) / bw
        ...
        errorUp = 1.0 + math.sqrt(math.pow((eUp / mc), 2))
        errorUp -= 1.0
        # errorDn = max(1. - math.sqrt(math.pow(tmpData.GetBinError(i)/mc,2) + math.pow((eDn/mc),2)),0)
        errorDn = max(1.0 - math.sqrt(math.pow((eDn / mc), 2)), 0)
        errorDn = 1.0 - errorDn
        # ratioGraph.SetPointY(i-1,1.)

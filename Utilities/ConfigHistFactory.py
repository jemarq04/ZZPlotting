...
class ConfigHistFactory(object):
    ...
    def readAllInSet(self, object_type, base_name):
        info = UserInput.readJson("/".join([self.manager_path, object_type, "%s.json" % base_name]))
        for file_name in glob.glob("/".join([self.manager_path, object_type, "%s_*.json" % base_name])):
...
def main():
    ...
    test = ConfigHistFactory(config["Setup"]["dataset_manager_path"], "ZZAnalysis", "Zselection")

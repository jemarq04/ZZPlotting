...
def loadHistFromChain(hist, file_list, path_to_tree, branch_name, cut_string, max_entries, append=False):
...
def loadHistFromTree(hist, root_file, path_to_tree, branch_name, cut_string, max_entries, append=False):
    if not root_file:
        print("Failed to open %s" % root_file)
        exit(0)
...
def loadHist(hist, tree, branch_name, cut_string, max_entries, append=False):
    ...
    num = tree.Draw(branch_name + ">>" + hist_name, cut_string, "", max_entries if max_entries > 0 else 1000000000)
...
def splitCanvas(oldcanvas, dimensions, ratio_text, ratio_range):
    ...
    for i in range(centralRatioHist.GetNbinsX() + 2):
        ...
        centralRatioHist.SetBinContent(i, 1.0)
...
def splitCanvasWithSyst(ratioband, oldcanvas, dimensions, ratio_text, ratio_range, isMassFull, varname):
    ...
    if "Mass" in varname and "Full" in varname:
        ...
        centralRatioHist.GetXaxis().ChangeLabel(1, -1, 0.0, -1, -1, -1, "")
    if "nJets" in varname:

...
def getComLineArgs():
    ...
    parser.add_argument("--unweighted", action="store_true", help="if provided, use 1 instead of cross-section*kfactor")
    parser.add_argument(
        "--scatter",
        action="store_true",
        help="if provided, the histograms will be plotted individuallyin a scatter plot instead of in a stacked plot",
    )
...
def getStacked(
    ...
    for plot_set in filelist:
        ...
        central_error = 0.0 if int(raw_events) <= 0 else error[0]
    ...
    central_error_tots = 0.0 if int(tot_sum.GetEntries()) <= 0 else error_tots[0]
    if "Mass" in branch_name:
        with open("CurrentRun_Event_output.txt", "a") as current_evt:
...
def main():
    ...
    config_factory = ConfigHistFactory(
        "%s/%s" % (path, manager_name), args.selection.split("_")[0], args.object_restrict
    )
            ...
            if not args.no_data:
                ...
                if args.hist_file == "":
                    ...
                    data_hist = helper.getConfigHistFromFile(
                        ...
                        lhe_weight_id=args.lhe_weight_id,
                    )

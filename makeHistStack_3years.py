...
def getComLineArgs():
    ...
    parser.add_argument(
        "--blinding",
        type=list,
        default=["Mass < 400", "Pt < 200", "mjj < 500", "dEtajj < 2.5", "MTWZ < 400"],
...
def writeMCLogInfo(hist_info, branch_name, latex):
    ...
    likelihood = 0 if weighted_events <= 0 else signal / math.sqrt(weighted_events)
    likelihood_err = (
        0
        if signal <= 0 or weighted_events <= 0
        else likelihood * math.sqrt((signal_err / signal) ** 2 + (0.5 * total_err / weighted_events) ** 2)
    )
    sigbkgd = 0 if weighted_events <= 0 else signal / weighted_events
    sigbkgd_err = (
        0
        if signal <= 0 or weighted_events <= 0
        else sigbkgd * math.sqrt((signal_err / signal) ** 2 + (total_err / weighted_events) ** 2)
    )
...
def getStacked(
    ...
    for plot_set in filelist:
        ...
        print(plot_set)
        if hist_file == "":
            hist = helper.getConfigHistFromTree(
                config_factory, plot_set, selection, branch_name, channels, luminosity, addOverflow, rebin, cut_string
            )
        else:
            hist = helper.getConfigHistFromFile(
                hist_file,
                config_factory,
                plot_set,
                selection,
        ...
        hist_info[plot_set] = {
            ...
            "stat error": 0 if raw_events <= 0 else weighted_events / math.sqrt(raw_events),
        }
    writeMCLogInfo(hist_info, branch_name, latex)
...
def main():
    # pdb.set_trace()
    # should run in 2018 script for current 3 year setting.
    args = getComLineArgs()
    ...
    config_factory = ConfigHistFactory(
        "%s/ZZ4lRun2DatasetManager" % path, args.selection.split("_")[0], args.object_restrict
    )
    ...
    for branch in branches:
        ...
        data_hists = []
        # pdb.set_trace()
        lumi3y = [35.9, 41.5, 59.7]
        fnameprefix = "/home/user1/cms_analysis/new_ZZ/CMSSW_10_3_1/src/ZZPlotting/HistFiles/"
        selectionsuffix = "/ZZSelectionsTightLeps"
        fname3y = ["16jetEtaHorn_all.root", "17jetEtaHorn_NewDatacombined.root", "18jetEtaHorn_all.root"]
        selections3y = ["ZZ4l2016", "ZZ4l2017", "ZZ4l2018"]
        for ind, lumi3yi in enumerate(lumi3y):
            args.selection = selections3y[ind] + selectionsuffix
            args.luminosity = lumi3yi
            args.hist_file = fnameprefix + fname3y[ind]
            ...
            for branch_name in branch.split("+"):
                ...
                try:
                    ...
                    hist_stack = getStacked(
                        selections3y[ind] + "stack_" + branch_name,
                        config_factory,
                        args.selection,
                        filelist,
                        branch_name,
                ...
                if not args.no_data:
                    ...
                    if args.hist_file == "":
                        ...
                        data_hist = helper.getConfigHistFromTree(
                            config_factory,
                            "data_all",
                            args.selection,
                            branch_name,
                        ...
                        data_hist = helper.getConfigHistFromFile(
                            args.hist_file,
                            config_factory,
                            "data_all",
                            args.selection,
                            branch_name,
                            args.channels,
                            addOverflow=(not args.no_overflow),
                            rebin=args.rebin,
                        )
                ...
                if ind > 0:
                    if not args.no_data:
                        data_hists[0].Add(data_hist)
                    for ind2, hist3y in enumerate(hist_stacks[0].GetHists()):
                        hist3y.Add(hist_stack.GetHists()[ind2])

...
def getStacked(name, config_factory, selection, filelist, branch_name, channels, _blinding, addOverflow, latex,
               cut_string="", luminosity=1, rebin=0, _uncertainties="none", hist_file=""):
    ...
    for plot_set in filelist:
        ...
        if hist_file == "":
            hist = helper.getConfigHistFromTree(config_factory, plot_set, selection,
                    branch_name, channels, luminosity, addOverflow, rebin, cut_string)
        else:
    ...
    for branch in branches:
        ...
        for ind,lumi3yi in enumerate(lumi3y):
            ...
            for branch_name in branch.split("+"):
                ...
                try:
                    #pdb.set_trace()
                    hist_stack = getStacked(selections3y[ind]+"stack_"+branch_name, config_factory, args.selection, filelist,
                ...
                if not args.no_data:
                    ...
                    if args.hist_file == "":
                        ...
                        data_hist = helper.getConfigHistFromTree(config_factory, "data_all", args.selection,
                                branch_name, args.channels, 1, not args.no_overflow, args.rebin, cut_string)
                    else:

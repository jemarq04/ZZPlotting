...
def invert2DHist(hist):
    new_hist = ROOT.TH2D(
        hist.GetName(), hist.GetTitle(), 4, 0, 2.5, 8, array.array("d", [5, 10, 20, 30, 40, 50, 60, 70, 200])
    )
...
def makeDataPlots(param, obj, outdir, chan):
    data_ewkcorr_graph = (
        frfile.Get("AllData/%slep1_%slep2_%s_%s" % (param[:2], param[:2], obj, chan))
        if "2D" in param
        else getTGraphAsymmErrors(frfile, "AllData", param, obj)
    )
...
def makeMCPlots(param, obj, outdir):
    graph = (
        frfile.Get("DYMC/ratio%s_all%s" % (param, obj))
        if "2D" in param
        else getTGraphAsymmErrors(frfile, "DYMC", param, obj)
    )
...
frfile = ROOT.TFile(
    "/data/uhussain/ZZTo4l/ZZ2018/VVAnalysisTools/CMSSW_9_4_2/src/Analysis/VVAnalysis/HistFiles/Hists29Nov2018-MuonTest.root"
)
